...
function Playoffs() {
    ...
    // Determine if home advantage applies for a given phase
    const phaseHasHomeAdvantage = (phaseKey) => {
        // Cuartos and Semis have home advantage; Final is neutral
        return phaseKey === 'cuartos' || phaseKey === 'semis';
    };

    const startLiveSimulation = (phaseKey, matchupId, legKey, localTeam, visitanteTeam, simId) => {
        const isHome = phaseHasHomeAdvantage(phaseKey);
        const result = simulateGame(
            localTeam?.['Team Name'] || 'Local', visitanteTeam?.['Team Name'] || 'Visitante', isHome,
            {
    ...
    const addMatchup = async (phaseKey, team1Id, team2Id) => {
        ...
        // Playoffs: single leg only, no BYE
        const newMatchup = {
            id: Date.now(),
            team1Id,
            ...
            isBye: false,
            winnerId: null,
            ida: { localId: team1Id, visitanteId: team2Id, localScore: null, visitanteScore: null, dateTime: null },
        };
    ...
    const updateLegBothScores = async (phaseKey, matchupId, legKey, localScore, visitanteScore, stats, scoringPlays, totalPlays, driveCount, broadcastTime, scoreByQuarter) => {
        ...
        const matchups = current[phaseKey].matchups.map(m => {
            ...
            updated.winnerId = computeWinner(updated);
            return updated;
        });
        current[phaseKey] = { ...current[phaseKey], matchups };
        await savePhases(current);
    ...
    // ── Winner computation (single leg only) ──
    const computeWinner = (matchup) => {
        ...
        // Tie: team1 wins (or could go to OT)
    ...
    if (loading) return <div className="loading">Cargando playoffs...</div>;

    const champion = getChampion();
    const currentMatchups = getPhaseMatchups(selectedPhase);

    // ── Build list of all leg cards (single leg per matchup, no aggregate needed) ──
    const legCards = [];
    ...
    return (
        <div className="cup-container">
            <h2 className="cup-title">PLAYOFFS</h2>
            <p className="cup-subtitle">Eliminación Directa · Partido Único</p>
            ...
            {champion && (
                <div className="cup-champion-banner">
                    ...
                    <div className="champion-title">Campeón Playoffs</div>
                                    ...
                                    return (
                                        <div key={`${matchup.id}-${legKey}`}>
                                            <div className="cup-matchup-header-bar">
                                                <span className="cup-matchup-label">
                                                    Llave {item.mIdx + 1}
                                                    {selectedPhase === 'final' && ' · Estadio Neutral'}
                                                </span>
                ...
                {showConfig && (
                    <div className="cup-add-form">
                        ...
                        <button className="cup-add-btn" onClick={() => {
                            ...
                            if (s1.value && s2.value) { addMatchup(selectedPhase, s1.value, s2.value); s1.value = ''; s2.value = ''; }
            ...
            {simulatingMatch && (() => {
                ...
                const isHome = phaseHasHomeAdvantage(simulatingMatch.phaseKey);
                ...
                return (
                    <GameSimulator
                        localTeam={localT} visitanteTeam={visitanteT} isLocalHome={isHome}
