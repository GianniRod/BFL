...
const PHASE_KEYS = ['cuartos', 'semis', 'final'];
const PHASE_NAMES = {
    octavos: 'Octavos',
    cuartos: 'Cuartos',
    semis: 'Semis',
...
function Playoffs() {
    ...
    const [selectedPhase, setSelectedPhase] = useState('cuartos');
    ...
    const addMatchup = async (phaseKey, team1Id, team2Id) => {
        ...
        const newMatchup = { id: Date.now(), team1Id, team2Id, isBye: false, winnerId: null };
    ...
    currentMatchups.forEach((matchup, mIdx) => {
        if (matchup.ida) legCards.push({ type: 'leg', matchup, mIdx, legKey: 'ida', label: null, leg: matchup.ida });
    });
    ...
    return (
        <div className="Playoffs-container">
            ...
            <p className="Playoffs-subtitle">Torneo de Eliminación Directa</p>

            <div className="Playoffs-year-selector">
                <label>Temporada:</label>
                <select value={selectedYear} onChange={(e) => setSelectedYear(parseInt(e.target.value))}>
                    {YEARS.map(y => <option key={y} value={y}>{y}</option>)}
                </select>
                <button className="Playoffs-config-btn" onClick={() => setShowConfig(!showConfig)}>
                    {showConfig ? 'Cerrar Config' : 'Configurar'}
                </button>
            </div>

            {champion && (
                <div className="Playoffs-champion-banner">
            ...
            <div className="Playoffs-carousel">
                <div className="Playoffs-carousel-nav">
                    <div className="Playoffs-phase-pills-strip">
                        {PHASE_KEYS.map(key => {
                            ...
                            return (
                                <button
                                    key={key}
                                    className={`Playoffs-phase-pill ${selectedPhase === key ? 'active' : ''} ${complete ? 'completed' : ''}`}
                ...
                <div className="Playoffs-view-toggle">
                    <button className={`Playoffs-view-btn ${viewMode === 'list' ? 'active' : ''}`} onClick={() => setViewMode('list')}>Lista</button>
                    <button className={`Playoffs-view-btn ${viewMode === 'bracket' ? 'active' : ''}`} onClick={() => setViewMode('bracket')}>Bracket</button>
                </div>

                {viewMode === 'list' ? (
                    <div className="fecha-content">
                        {matchupGroups.length === 0 && (
                            <div className="Playoffs-empty-state">
                        ...
                        {matchupGroups.map((group, gIdx) => (
                            <div key={group.matchupId} className="Playoffs-matchup-group">
                                {group.items.map((item, idx) => {
                                            ...
                                            <div key={`bye-${item.matchup.id}`} className="partido-card" style={{ opacity: 0.55, borderStyle: 'dashed' }}>
                                                <div className="partido-row">
                                                    ...
                                                    <div className="partido-center">
                                                        <div className="Playoffs-bye-text" style={{ color: '#4caf50', fontWeight: 600, fontSize: '0.8rem' }}>
                                                            BYE – Clasificado
                                                        </div>
                                                    </div>
                                                    <div className="partido-right-side">
                                                        {showConfig && (
                                                            <button className="Playoffs-remove-matchup-btn" onClick={() => removeMatchup(selectedPhase, item.matchup.id)}>✕</button>
                                                        )}
                                                    </div>
                                                </div>
                                            </div>
                                    ...
                                    if (item.type === 'aggregate') {
                                        ...
                                        return (
                                            <div key={`agg-${item.matchup.id}`} className="Playoffs-aggregate-bar">
                                    ...
                                    const simId = `Playoffs_${matchup.id}_${legKey}`;
                                    const liveData = liveMatchesUI[simId];

                                    return (
                                        <div key={`${matchup.id}-${legKey}`}>
                                            {legKey === 'ida' && (
                                                <div className="Playoffs-matchup-header-bar">
                                                    <span className="Playoffs-matchup-label">Llave {item.mIdx + 1}{label ? ` · ${label}` : ''}</span>
                                                    <div className="Playoffs-matchup-badge">
                                                        {matchup.winnerId && (() => {
                                                        ...
                                                        {showConfig && <button className="Playoffs-remove-matchup-btn" onClick={() => removeMatchup(selectedPhase, matchup.id)}>✕</button>}
                                                    </div>
                                                </div>
                                            )}
                                            {legKey === 'vuelta' && (
                                                <div className="Playoffs-matchup-header-bar">
                                                    <span className="Playoffs-matchup-label">Llave {item.mIdx + 1} · {label}</span>
                                                    <span></span>
                                                </div>
                                            )}
                                    ...
                                    return (
                                        <div
                                            ...
                                            onClick={() => {
                                                ...
                                                const legKey = 'ida';
                                                const simId = `Playoffs_${m.id}_${legKey}`;
                ...
                {/* Add matchup form */}
                {showConfig && (
                    <div className="Playoffs-add-form">
                        <select id={`Playoffs-t1-${selectedPhase}`} defaultValue="">
                        ...
                        <select id={`Playoffs-t2-${selectedPhase}`} defaultValue="">
                            <option value="" disabled>Equipo 2</option>
                            {getAvailableTeams(selectedPhase).map(t => <option key={t.id} value={t.id}>{t['Team Name']}</option>)}
                        </select>
                        <button className="Playoffs-add-btn" onClick={() => {
                            const s1 = document.getElementById(`Playoffs-t1-${selectedPhase}`);
                            const s2 = document.getElementById(`Playoffs-t2-${selectedPhase}`);
